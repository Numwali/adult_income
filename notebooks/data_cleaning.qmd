...
```{python}
#| label: tbl-info
#| tbl-cap: DataFrame column summary

import pandas as pd

summary = pd.DataFrame({
    'Column': adult_df.columns,
    'Non-Null Count': adult_df.notnull().sum().values,
    'Dtype': adult_df.dtypes.astype(str).values
})
summary
```
...
```{python}
#| label: tbl-missing
#| tbl-cap: Count of missing values per column

missing = adult_df.isnull().sum().reset_index()
...
```{python}
#| label: tbl-missing-after-impute
#| tbl-cap: Missing values and percentage per column after imputation

missing_after = pd.DataFrame({
    'Column': adult_df.columns,
    'Missing Values': adult_df.isnull().sum().values,
    'Percent Missing': (adult_df.isnull().mean().values * 100).round(2)
})
missing_after
```
...
```{python}
#| label: tbl-object-dtypes
#| tbl-cap: Columns with object dtype

object_dtypes = pd.DataFrame({
    'Column': adult_df.columns,
    'Is Object Dtype': (adult_df.dtypes == 'object').values
})
object_dtypes
```
...
```{python}
#| label: tbl-final-missing-check
#| tbl-cap: Final check for missing values per column

final_missing = adult_df.isna().sum().reset_index()
final_missing.columns = ['Column', 'Missing Values']
final_missing
```
