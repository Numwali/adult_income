...
display(Image(os.path.join(results_dir, 'income_distribution_pie_chart.png')))
```
...
display(Image(os.path.join(results_dir, 'income_distribution_by_agegroup_bar_plot.png')))
```
...
display(Image(os.path.join(results_dir, 'income_distribution_by_nativeregion_bar_plot.png')))
```
...
display(Image(os.path.join(results_dir, 'income_distribution_by_race_bar_plot.png')))
```
...
display(Image(os.path.join(results_dir, 'income_distribution_by_eduandocc_bar_plot.png')))
```
...
Tertiary education combined with white-collar work offers the highest income prospects. Yet a substantial number of tertiary-educated white-collar workers earn less than $ 50,000, likely due to early career, part-time, or structural pay gaps.

- **Blue-collar and service work predominantly pay <=50K, regardless of education.**

Even a college education doesn’t guarantee high incomes in these sectors. Manual and service sector income is highly occupation-dependent (some skilled trades can break the 50K mark).

- **Some non-tertiary education groups do reach >50K**

Secondary-school graduates in blue-collar and white-collar occupations have decent representation among those earning over $50,000. This reflects upward mobility possible through skilled trades, tenure, or niche roles.
